...
- StudioNet contract: `0xCD7E3bad31F1C26F139907621A569940C2AD70Bd`
- Contract version: `3.1.1`
- Frontend release: `3.4.0`
...
Frontend release 3.4 pins the app to the typechecked canonical StudioNet deployment and verifies its complete public schema before allowing a wallet signature. It normalizes both typed camel-case receipts and the raw snake-case lifecycle fields returned by StudioNet, including the leader execution result. Only `deposit` carries GEN; the five-argument `create_bounty` and every other business action omit transaction value and spend recoverable app credit. Mobile protocol data is constrained to the viewport, including full contract and treasury addresses.

The canonical release was exercised with the same `genlayer-js` adapter used by the web app: deposit `0x3de624bdf84ab0f8dcbf4c318cb10756fd7eb96cd10884906a70b77b0a71137a`, zero-value creation `0x10e4607dbc7a53f9b162d824776456b653b3c592694deed72d5d50e4b06e660b`, and cancellation/refund `0x8f09c329ae1d80cd523814c84d64f44cfbd4d7c2b9ed49192b2ede55e69d1202` all finalized with successful execution.

A separate two-wallet acceptance run completed the full workflow on this exact production contract using public [issue #3](https://github.com/Demigodd00/bountyforge/issues/3) and [PR #4](https://github.com/Demigodd00/bountyforge/pull/4). Bounty `bf-4` progressed through funding, zero-value creation, claim registration, immutable evidence capture, a 90-confidence `FIXES_ISSUE` verdict, the five-minute challenge window, finalization, and hunter payout. Its final state is `SETTLED`; claim 0 is `PAID`; both participant app-credit balances are zero and the lifecycle returned the shared contract balance to its pre-run baseline.
