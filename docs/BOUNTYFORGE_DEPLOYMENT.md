...
- Contract: `0xCD7E3bad31F1C26F139907621A569940C2AD70Bd`, version `3.1.1`, on StudioNet.
...
The release 3.4 remediation and its verification limits are tracked in [BOUNTYFORGE_REVIEW.md](BOUNTYFORGE_REVIEW.md). A frontend build alone is not approval: verify the current contract version, recoverable funding model, source, and live balance behavior before activating a new address.
...
The repository includes `apps/bountyforge-web/vercel.json` with the install and build settings. The explicit Corepack commands use the app-level package-manager pin when the enclosing repository has no root `package.json`. The project `demi17/bountyforge-web` is linked to the repository, and its Root Directory is configured for Git-based production builds from `main`.
...
$env:NEXT_PUBLIC_BOUNTYFORGE_ADDRESS="0xCD7E3bad31F1C26F139907621A569940C2AD70Bd"
...
Do not send GEN directly to `create_bounty`, `submit_claim`, or `challenge_claim`: they are nonpayable in v3.1.1. The only payable method is `deposit()`. Failed payable operations on StudioNet were observed to retain attached funds, which is why the release uses this separate, minimal funding entry point. Once a transaction hash has been returned, an unknown confirmation result retains that public hash and blocks another write until its receipt or exact finalized state is checked. Never resubmit merely because StudioNet receipt polling is delayed. If the wallet fails before returning a hash, inspect its activity before retrying.
...
genvm-lint typecheck contracts/bounty_forge.py
...
The production frontend adapter has been live-tested for deposit, zero-value creation, finalized state, and cancellation/refund. A separate two-wallet run completed claim registration, GenLayer adjudication, challenge-window finalization, and payout on the same canonical contract. These checks establish StudioNet submission readiness; they are not an independent security audit or a guarantee of financial safety.
