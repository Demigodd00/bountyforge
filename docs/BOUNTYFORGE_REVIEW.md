...
| Vercel auto-deploys came from the older workspace repository | Reconnected project `bountyforge-web` to `Demigodd00/bountyforge`, branch `main`, root `apps/bountyforge-web`, then restored the verified release alias | Vercel project API reports repository ID `1350586793`; all six production routes return HTTP 200 |
